# Demo Video Links — Agent Skills (Task 3)

Promoted from `hw3/work/task-3-cross-platform/demo-video-links-draft.md`.
...
## What the session records confirm

- Both skills ran for real against the actual Task 3 artifacts during the
  recorded session — evidenced by a backup file created specifically at
  that time (`hw3/work/task-3-cross-platform/demo-workspace/report-draft.backup-pre-video.md`)
  before `report-draft.md` was regenerated again.
...
- Skill 2 output: `report-draft.md` refreshed with all 12 required sections,
  Blocked never labeled Fail, output confirmed to remain only inside
  `hw3/work/`.
- No discrepancy was triggered during the recorded session; the plan's
  failure/recovery branch was not exercised.
