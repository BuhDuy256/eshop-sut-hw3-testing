...
## BUG-17-001 — `code` has no required-ness validation
...
**Steps to reproduce:**
1. Login as admin.
2. `POST /api/admin/coupons` with no `code` key, other fields valid.
...
`POST /api/admin/coupons` (lines 457–481) destructures `code` from `req.body` and binds it
straight into the parameterized `INSERT` with no presence check; `coupons.code` (database.js
line 31) is `TEXT UNIQUE` but not `NOT NULL`.

**Evidence:** [`evidence/fr17-raw-execution-log.txt`](evidence/fr17-raw-execution-log.txt),
lines 31–41 (raw request/response capture, API-level bug, no browser involved).

---

## BUG-17-002 — `type` has no validation (accepts non-enum values and omission)
...
**Evidence:** [`evidence/fr17-raw-execution-log.txt`](evidence/fr17-raw-execution-log.txt),
lines 43–65 (`TC-17-EP-004`/`005`), 287–292 (`TC-17-BVA-015`).

---

## BUG-17-003 — `discount_value` has no validation (accepts zero and negative values)
...
**Evidence:** [`evidence/fr17-raw-execution-log.txt`](evidence/fr17-raw-execution-log.txt),
lines 67–77 (`TC-17-EP-006`), 189–201 (`TC-17-BVA-001`/`002`).

---

## BUG-17-004 — `expired_at` has no validation (accepts omission and an empty value)
...
**Evidence:** [`evidence/fr17-raw-execution-log.txt`](evidence/fr17-raw-execution-log.txt),
lines 79–89 (`TC-17-EP-007`), 294–299 (`TC-17-BVA-016`).

---

## BUG-17-005 — `min_order_amount` has no validation (accepts negative values and omission)
...
**Evidence:** [`evidence/fr17-raw-execution-log.txt`](evidence/fr17-raw-execution-log.txt),
lines 91–113 (`TC-17-EP-008`/`009`), 210–215 (`TC-17-BVA-004`).

---

## BUG-17-006 — `max_uses_per_user`'s fallback protects against falsy invalid input but not negative values
...
## BUG-17-008 — `POST /api/admin/coupons` has no access control (any authenticated user can create coupons)
...
this pass (a scope decision recorded in the domain-testing report, mirroring FR-15's identical
reasoning: all three coupon routes share the exact same middleware shape, so this sub-state was
tested once on View and once on Create, not repeated a third time). Given `BUG-17-007`/`008`
above, it is highly likely `DELETE` has the same gap — but per MODEL ≠ ORACLE, this is stated
as a strong suspicion, not filed as a confirmed defect, since it was never actually executed.
...
**GitHub filing:** all 8 bugs filed verbatim as GitHub issues
...
2026-07-07, immediately after human approval — see
`work/FR-17-coupon-crud/bug-report-drafts.md` for the approval record.
